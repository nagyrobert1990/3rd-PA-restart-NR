...
SELECT users.email AS user_email,
	...
	ins_companies.company_name AS company_name,
	ins_services.service_name AS service_name,
	ins_bonds.issued_date AS date_of_issue
	FROM sign_a_bond
	JOIN users ON sign_a_bond.user_id = users.id
	JOIN cars ON sign_a_bond.car_id = cars.id
	JOIN ins_companies ON sign_a_bond.company_id = ins_companies.id
	JOIN ins_services ON sign_a_bond.service_id = ins_services.id
	JOIN ins_bonds ON sign_a_bond.bond_id = ins_bonds.id
	WHERE ins_bonds.issued_date < 2018;
COMMIT;
